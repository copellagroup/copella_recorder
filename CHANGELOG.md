# Copella Recorder - Обновления

## Версия 1.1.0 - Новые микрофункции

### Новые функции:
1. **Волновая анимация** - `createWaveAnimation()` - эффект волн для кнопок и элементов
2. **Эффект печатной машинки** - `addTypewriterEffect()` - анимированный набор текста

### Обновления:
- Обновлена приветственная модалка с информацией о новых функциях
- Показывается при первом запуске версии 1.1

---

## Версия 1.0.0 - Gilroy Edition
...
3. **Новые микрофункции**
   ...
   - `createNotificationBadge()` - значки уведомлений
   - `addPulseEffect()` - эффект пульсации
   - `createTooltip()` - всплывающие подсказки
   - `addGlowEffect()` / `removeGlowEffect()` - эффекты свечения
   - `createLoadingSpinner()` - спиннеры загрузки
   - `addShakeEffect()` - эффект тряски

4. **Улучшения UI**
   - Добавлены эффекты ripple для всех кнопок
   - Строгий минималистичный дизайн
   - Убраны все цветные кружочки и градиенты
   - Красный эквалайзер при записи
   - Улучшена толщина шрифтов (font-semibold вместо font-medium)
