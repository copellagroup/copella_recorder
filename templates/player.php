...
<div class="app-container flex flex-col h-full max-w-4xl mx-auto w-full p-4 sm:p-6 pb-[calc(1rem+env(safe-area-inset-bottom))] bg-bg-color text-text-primary font-gilroy min-h-screen antialiased">
    ...
    <header class="app-header fixed top-0 left-0 right-0 z-50 flex justify-between items-center h-16 sm:h-18 px-4 bg-[#1a1a1a] border-b border-white/10 font-gilroy">
        ...
        <div class="header-actions flex items-center gap-2">
            <span id="mskTime" class="text-sm font-semibold text-text-secondary hidden sm:block"></span>
    ...
    <main class="flex flex-col flex-grow overflow-hidden pt-16 sm:pt-18">
        <section id="playerPanel" class="player-panel relative bg-panel-bg rounded-large p-6 mt-4 flex-shrink-0 overflow-hidden">
            ...
            <div class="absolute top-4 right-4 z-20 flex gap-2">
                <button id="statsBtn" class="action-btn p-2 rounded-full text-text-secondary hover:bg-white/10 hover:text-accent transition-colors" title="Статистика"><svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M16,6L18.29,8.29L13.41,13.17L9.41,9.17L2,16.59L3.41,18L9.41,12L13.41,16L19.71,9.71L22,12V6H16Z"></path></svg></button>
                <button id="streamInfoBtn" class="action-btn p-2 rounded-full text-text-secondary hover:bg-white/10 hover:text-accent transition-colors" title="Информация о потоке"><svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"></path></svg></button>
            </div>
        ...
        <section class="stations-panel mt-6 flex-grow flex flex-col overflow-hidden">
            <div id="stationsHeader" class="stations-header flex justify-between items-center mb-4 flex-shrink-0">
                <div class="search-container relative flex-1 mr-2 sm:mr-4">
                    ...
                    <input type="search" id="stationSearchInput" placeholder="Найти станцию..." class="w-full bg-panel-bg border-none rounded-small py-2.5 pl-10 pr-10 text-text-primary text-base outline-none focus:ring-2 focus:ring-accent/50 transition-all">
                    <button id="clearSearchBtn" class="absolute right-3 top-1/2 -translate-y-1/2 text-text-secondary hover:text-accent transition-colors hidden" title="Очистить поиск">
                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M6 6l12 12M18 6L6 18"></path></svg>
                    </button>
                </div>
                ...
                <div id="apiSearchResults" class="hidden">
                    <div class="flex items-center justify-between mb-3 px-2">
                        ...
                        <div class="flex items-center gap-2">
                            <button id="apiSearchPrev" class="p-1 rounded-full text-text-secondary hover:bg-white/10 hover:text-accent transition-colors" title="Предыдущие">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M15 6l-6 6 6 6"/></svg>
                            </button>
                            <button id="apiSearchNext" class="p-1 rounded-full text-text-secondary hover:bg-white/10 hover:text-accent transition-colors" title="Следующие">
