window.CopellaPlayer = (function(){
  ...
  function createPlayerHTML(station) {
    ...
    return '<div class="player-art w-32 h-32 rounded-large flex-shrink-0 bg-bg-color overflow-hidden transition-shadow duration-400 ease-in-out">' + artContent + '</div>' +
      ...
      '<div id="player-now-playing" class="text-base font-semibold text-text-secondary transition-colors duration-300 whitespace-nowrap overflow-hidden text-ellipsis w-full">Прямой эфир</div>' +
  ...
  function renderPlayer(station) {
    if (station) {
      ...
      document.getElementById('recordButton').onclick = function(e){ 
        CopellaUI.haptic(); 
        CopellaUI.addRippleEffect(e.target, e);
        CopellaRecording.toggleRecording(); 
      };
      document.getElementById('playPauseButton').onclick = function(e){ 
        CopellaUI.haptic(); 
        CopellaUI.addRippleEffect(e.target, e);
        togglePlayPause(); 
      };
      document.getElementById('copyUrlBtn').onclick = function(e){ 
        e.stopPropagation(); 
        CopellaUI.haptic(); 
        CopellaUI.addRippleEffect(e.target, e);
        navigator.clipboard.writeText(station.url).then(function(){ 
          CopellaUI.showToast('URL потока скопирован!', 'success'); 
        }); 
      };
      ...
      volumeIcon.onclick = function(e){ 
        CopellaUI.addRippleEffect(e.target, e);
        CopellaDOM.audioPlayer.muted = !CopellaDOM.audioPlayer.muted; 
        updateVolumeUI(); 
      };
      updatePlayPauseIcon();
      updateVolumeUI();
